...
public class Solution216 {
    ...
    public List<List<Integer>> combinationSum3(int k, int n) {
        ...
        process(k, n, new LinkedList<Integer>(), 0, vis);
        List<List<Integer>> collect = res.stream().peek(x->x.sort((o1, o2) -> o1.compareTo(o2))).distinct().collect(Collectors.toList());
        return collect;
    }
    List<List<Integer>> res = new ArrayList<>();
    private void process(int k, int n, LinkedList<Integer> integers, int sum, boolean[] vis) {
        if (sum > n) {
            return;
        }
        if (k == 0) {
        ...
        for (int i = 1; i <= 9; i++) {
            ...
            integers.addLast(i);
            vis[i]=true;
            process(k-1,n,integers,sum+i,vis);
            vis[i]=false;
            integers.pollLast();
        }
    }
