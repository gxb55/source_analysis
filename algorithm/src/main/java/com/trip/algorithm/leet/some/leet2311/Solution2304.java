    ...
    public static void main(String[] args) {
     ...
     //  int[][] grid = {{5,1,2},{4,0,3}}, moveCost = {{12,10,15},{20,23,8},{21,7,1},{8,1,13},{9,10,25},{5,3,2}};
       int[][] grid = {{28,35,29,5,13,17,18,23,14},{30,15,12,27,2,26,25,19,7},{1,16,34,21,9,3,20,24,8},{4,33,22,11,31,0,6,10,32}}
        ...
        int i = solution2304.minPathCost(grid, moveCost);
    ...
    public int minPathCost(int[][] grid, int[][] moveCost) {
        ...
        for (int i = 1; i <dp.length ; i++) {
            for (int j = 0; j < dp[i].length; j++) {
                int res = getRes(dp[i - 1], moveCost, i, j, grid);
                dp[i][j]=res+grid[i][j];
            }
        }

        int asInt = Arrays.stream(dp[xLen - 1]).min().getAsInt();
        return asInt;
    }

    private int getRes(int[] ints, int[][] moveCost, int x, int y, int[][] grid) {
        int min=Integer.MAX_VALUE;
        for (int i = 0; i < ints.length; i++) {
            int val1 = moveCost[grid[x - 1][i]][y];
            int val2 = ints[i];
