...
public class DistributedLock1 {
    ...
    // 和 zk 服务建立连接，并创建根节点
    public DistributedLock1() throws IOException,
            InterruptedException, KeeperException {
        zk = new ZooKeeper(host, sessionTimeout, new
                Watcher() {
                    @Override
    ...
    public static void main(String[] args) throws Exception {
        // 创建分布式锁 1
        final DistributedLock1 lock1 = new DistributedLock1();
        // 创建分布式锁 2
        final DistributedLock1 lock2 = new DistributedLock1();
        final DistributedLock1 lock3 = new DistributedLock1();
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    lock3.zkLock();
                    System.out.println("线程 3 获取锁");
                    Thread.sleep(5 * 1000);
                    lock3.zkUnlock();
                    System.out.println("线程 3 释放锁");
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
