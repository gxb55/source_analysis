...
/**
 * @author xbguo
 * @createTime 2022年07月02日 08:40:00
 * 871. 最低加油次数
...
public class Solution871 {
    public static void main(String[] args) {
      ...
      /*  int target = 1, startFuel = 1;
       ...
       /* int target = 100, startFuel = 10;
        ...
        int i = solution871.minRefuelStops(target, startFuel, stations);
    ...
    public int minRefuelStops(int target, int startFuel, int[][] stations) {
        ...
        int ans = 0;
        while (cur < target) {
            if (index < len && cur >= stations[index][0]) {
                priorityQueue.add(stations[index][1]);
                index++;
            } else {
                if (!priorityQueue.isEmpty()) {
                    cur = cur + priorityQueue.poll();
                    ans++;
                } else {
                    return -1;
                }
    ...
    public int minRefuelStops1(int target, int startFuel, int[][] stations) {
        ...
        for (int i = 0; i < len; i++) {
            for (int j = i; j >=0; j--) {
                if ((dp[i] - stations[j][0]) >= 0) {
                    dp[i + 1] = Math.max(dp[i + 1], dp[i] + stations[j][1]);
                }
            }
        }
